    ...
    <div class="card mt-4">
      <!-- Button for making new user -->  
        <div class="card-header">
            <h3 class="card-title">Edit Contact Us Texts</h3>
        </div>
        <form action="{{ route('admin.contact_us.texts.update', $contact_text->id) }}" method="POST" enctype="multipart/form-data">
        @csrf
        <div class="mb-3 m-3">
            <label for="title_1" class="form-label">{{ __('Title-1:') }}</label>
            <input type="text" name="title_1" id="title_1" required class="form-control" value="{{$contact_text->title_1}}">
            @error('title_1')
        ...
        <div class="mb-3 m-3">
            ...
            <input type="text" name="title_2" id="title_2" required class="form-control" value="{{$contact_text->title_2}}">
            @error('title_2')
                <span class="invalid-feedback" role="alert">
                    <strong>{{ $message }}</strong>
                </span>
        ...
        <div class="mb-3 m-3">
            ...
            <input type="text" name="title_3" id="title_3" required class="form-control" value="{{$contact_text->title_3}}">
            @error('title_3')
                <span class="invalid-feedback" role="alert">
                    <strong>{{ $message }}</strong>
                </span>
        ...
        <input type="hidden" name="lang" value="{{$contact_text->language->name}}">
